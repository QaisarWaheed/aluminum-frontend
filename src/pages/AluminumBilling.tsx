      ...
      <Stack gap="md">
        ...
        >
          ...
          <Flex wrap="wrap" gap="md">
            <TextInput type="number" label="Invoice No" name="invoiceNo" value={formData.invoiceNo || ""} onChange={handleCustomerChange} disabled style={{ flex: "1 1 150px" }} />
            <TextInput label="Customer Name" name="customerName" value={formData.customerName} onChange={handleCustomerChange} style={{ flex: "1 1 200px" }} />
            <TextInput type="date" label="Date" name="date" value={formData.date} onChange={handleCustomerChange} style={{ flex: "1 1 200px" }} />
            <TextInput label="Company Name" name="companyName" value={formData.companyName} onChange={handleCustomerChange} style={{ flex: "1 1 200px" }} />
            <TextInput label="City" defaultValue={"Multan"} style={{ flex: "1 1 150px" }} />
          </Flex>
            ...
            <Table withTableBorder highlightOnHover style={{ minWidth: 900 }}>
              ...
              <tbody>
                {formData.products.map((item, index) => (
                  <tr key={item.id}>
                    <td>{index + 1}</td>
                    <td><TextInput value={item.section} onChange={(e) => handleItemChange(item.id, "section", e.currentTarget.value)} /></td>
                    <td><TextInput value={item.size} onChange={(e) => handleItemChange(item.id, "size", e.currentTarget.value)} /></td>
                    ...
                    <td><Select data={["0.9", "1.1", "1.2", "1.4", "1.6", "2.0"]} value={item.gaje} onChange={(value) => handleItemChange(item.id, "gaje", value || "")} checkIconPosition="right" /></td>
                    <td><Select data={["CH","BLM","WT","SL","WOOD","SAHARA","MALTI"]} value={item.color} onChange={(value) => handleItemChange(item.id, "color", value || "")} checkIconPosition="right" allowDeselect /></td>
                    <td><TextInput type="number" value={item.rate} onChange={(e) => handleItemChange(item.id, "rate", Number(e.currentTarget.value))} /></td>
                    <td><TextInput type="number" value={item.discount} onChange={(e) => handleItemChange(item.id, "discount", Number(e.currentTarget.value))} /></td>
          ...
          <Flex wrap="wrap" gap="md" mt="md">
            <Box p="md" style={{ backgroundColor: "#f9f9f9", borderRadius: 8, border: "1px solid #eee", flex: "1 1 100%" }}>
              <Flex wrap="wrap" gap="md" style={{ justifyContent: "space-between" }}>
                <div style={{ backgroundColor: "#f9f9f9", border: "1px solid #eee", padding: "5px", flex: "1 1 120px", minWidth: 120 }}>
                  <strong>Discounted Amount:</strong> Rs. {discountedAmount.toFixed(2)}
                </div>
                <TextInput label="Previous Amount" type="number" value={formData.previousAmount} onChange={handleCustomerChange} name="previousAmount" bg={"#f9f9f9"} placeholder="0.00" style={{ flex: "1 1 120px", minWidth: 120 }} />
                <TextInput label="Hardware Total" type="number" name="hardwareAmount" value={formData.hardwareAmount} onChange={handleCustomerChange} bg={"#f9f9f9"} placeholder="0.00" style={{ flex: "1 1 120px", minWidth: 120 }} />
                <div style={{ backgroundColor: "#f9f9f9", border: "1px solid #eee", padding: "5px", flex: "1 1 120px", minWidth: 120 }}>
                  <strong>Total Amount:</strong> Rs. {total.toFixed(2)}
                </div>
                <TextInput label="Received Amount" placeholder="0.00" type="number" name="receivedAmount" value={formData.receivedAmount} onChange={handleCustomerChange} bg={"#f9f9f9"} style={{ flex: "1 1 120px", minWidth: 120 }} />
                <div style={{ backgroundColor: "#f9f9f9", border: "1px solid #eee", padding: "5px", flex: "1 1 120px", minWidth: 120 }}>
        ...
        <Flex wrap="wrap" justify="center" gap="md" mt="xl">
          ...
          <Button onClick={() => navigate("/aluminum-bills")} p={10}>A-Bill Save</Button>
          <Button onClick={() => navigate("/hardware-bills")} p={10}>H-Bill Save</Button>
        </Flex>
      </Stack>
    </Box>
  );
